...
inline scalar linearInterpolation::interpolate(const List<int>& ub, const scalarList& pos) const
{
   ...
   // for progress variable			//change Chi to C
...
inline scalarList linearInterpolation::interpolateS(const List<scalarList>& params_, const List<int>& ub, const scalarList& pos) const
{
   scalarList b0_(params_[0].size(),0.0);
   scalarList b1_(params_[0].size(),0.0);
   scalarList b_(params_[0].size(),0.0);
   for  (register label k = 0; k < params_[0].size(); k++)
{
   b0_[k] = tableValues_[ub[0] -1][ub[1] -1][k]*(1-pos[0]) + tableValues_[ub[0]][ub[1] -1][k]*pos[0];
   b1_[k] = tableValues_[ub[0] -1][ub[1]][k]*(1-pos[0]) + tableValues_[ub[0]][ub[1]][k]*pos[0];
   b_[k] = b0_[k]*(1-pos[0]) + b1_[k]*pos[0];
}
